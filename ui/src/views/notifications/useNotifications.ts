...
export function useNotifications(visible: boolean) {
    ...
    (reason: string) => {
      ...
      change((value) => ({
        ...
        error: Object.values(value.pending).some((pending) => pending !== "accepted")
          ? "Connection lost; the dismissal may have reached your iPhone. " +
  ...
  const refresh = useCallback(() => {
    ...
    void sendDaemonCommand({ command: "bt_list_notifications" }).catch(() => {
      if (epoch === connectionEpoch.current && visibleRef.current) {
        change((value) => ({
          ...value,
          error: "Could not refresh notifications. Check the tetherd connection and try again.",
        }));
      }
    });
  }, [change]);
        ...
        case "bt_notifications": {
          ...
          change((value) => ({
            ...
            pending: Object.fromEntries(
              Object.entries(value.pending).filter(([uid]) => present.has(Number(uid))),
            ),
          }));
          break;
        }
        ...
        case "bt_notification_removed":
          ...
          change((value) => {
            ...
            return {
              ...value,
              pending,
              items: value.items.filter((item) => item.uid !== event.uid),
        ...
        case "bt_notification_action_result":
          ...
          change((value) => {
            ...
            return {
              ...
              error: event.success
                ? ""
                : "The iPhone did not confirm the dismissal. Check it before trying again.",
            };
          });
  ...
  const dismiss = (uid: number) => {
    ...
    if (
      !value.ready ||
      value.pending[uid] ||
      !value.items.some((item) => item.uid === uid && item.negative_action)
    )
      return;

    change((previous) => ({
      ...previous,
      pending: { ...previous.pending, [uid]: "waiting" },
      error: "",
    }));
    timers.current.set(
      uid,
      window.setTimeout(() => {
        ...
        change((previous) => ({
          ...previous,
          pending: { ...previous.pending, [uid]: "uncertain" },
      ...
      () => {
        if (current.current.pending[uid] === "waiting") {
          change((previous) => ({
            ...previous,
            error:
              "Could not confirm the dismissal request. Wait for a result or check your iPhone.",
          }));
        }
      },
    );
