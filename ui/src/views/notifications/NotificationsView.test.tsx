...
function connect(available = true) {
  act(() => {
    ...
    FakeEventSource.current.emit({
      ...
      version: 1,
      capabilities: available ? ["notifications"] : [],
    });
  });
}
...
describe("Notifications view", () => {
  it("gates an unavailable daemon and explains ANCS permission recovery", () => {
    ...
    expect(
      screen.getByText("Enable Show Notifications in iPhone Bluetooth settings."),
    ).toBeInTheDocument();
  ...
  it("shows source and content, and keeps attempted dismissals disabled until removal", () => {
    ...
    expect(screen.getByRole("alert")).toHaveTextContent(
      "The iPhone did not confirm the dismissal.",
    );
    expect(dismiss).toBeDisabled();

    const reminder = screen.getByRole("button", {
      name: "Dismiss Reminders notification on iPhone",
    });
  ...
  it("hides old content when ANCS becomes unavailable, including delayed list results", () => {
    ...
    act(() => {
      ...
      FakeEventSource.current.emit({
        command: "bt_notifications",
        notifications: [{ uid: 1, title: "Private text" }],
      });
    });
  ...
  it("clears private rows on disconnect and shows an empty state after refresh", () => {
    ...
    act(() => {
      ...
      FakeEventSource.current.emit({
        command: "bt_notifications",
        notifications: [{ uid: 1, app_name: "Mail", title: "Secret" }],
      });
    });
    ...
    act(() =>
      FakeEventSource.current.emit({
        command: "bt_notifications",
        notifications: [{ uid: 1, title: "Secret" }],
      }),
    );
    expect(screen.queryByText("Secret")).not.toBeInTheDocument();
    act(() => {
