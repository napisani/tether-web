...
describe("Notifications lifecycle", () => {
  it("refreshes when visible, after ANCS connects, and on live changes", () => {
    const { result, rerender } = renderHook(({ visible }) => useNotifications(visible), {
    ...
    expect(
      commands().filter((command) => command.command === "bt_list_notifications"),
    ).toHaveLength(2);
  ...
  it("does not restore a removed UID from an older list snapshot", () => {
    ...
    act(() =>
      result.current.handleEvent({
        ...
        notifications: [item, { uid: 43, title: "Another notification" }],
      }),
    );
  ...
  it("scopes dismissals by UID and waits for authoritative removal", () => {
    ...
    act(() => result.current.dismiss(42));
    expect(
      commands().filter((command) => command.command === "bt_notification_action"),
    ).toHaveLength(1);
    act(() =>
      result.current.handleEvent({
