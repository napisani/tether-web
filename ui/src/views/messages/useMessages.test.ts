...
describe("Messages lifecycle", () => {
  it("does not accept retained thread data before MAP readiness", () => {
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_threads",
        threads: [{ thread, name: "Current phone" }],
      }),
    );
    expect(result.current.state.threads[0].name).toBe("Current phone");
  });

  it("loads visible threads and marks selected unread handles once", async () => {
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_threads",
        threads: [{ thread, name: "Ada", unread: 1 }],
      }),
    );
    ...
    act(() =>
      result.current.handleEvent({
        ...
        read: true,
        success: false,
        message: "MAP denied read",
      }),
    );
    expect(result.current.state.error).toBe("MAP denied read");
    act(() => result.current.handleEvent({ command: "bt_messages", thread, messages: [message] }));
    await waitFor(() =>
      expect(commands().filter((command) => command.command === "bt_mark_read")).toHaveLength(2),
    );
    act(() =>
      result.current.handleEvent({
        command: "bt_message_read",
        ...
        read: true,
        success: true,
        message: "One of the reads failed",
      }),
    );
  ...
  it("resolves an existing contact thread when Messages has never been opened", () => {
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_threads",
        threads: [{ thread, name: "Previous phone" }],
      }),
    );
    ...
    expect(commands()).toContainEqual({ command: "bt_list_threads" });
    act(() =>
      result.current.handleEvent({ command: "bt_threads", threads: [{ thread, name: "Ada" }] }),
    );
  ...
  it("opens existing and new contact addresses without losing namespaced thread keys", () => {
    ...
    expect(result.current.state).toMatchObject({
      selected: "email:grace@example.com",
      recipient: "Grace",
      composing: true,
    });
    expect(commands()).toContainEqual({
      command: "bt_list_messages",
      thread: "email:grace@example.com",
    });
  });
  ...
  it("only an owned result clears its draft, and a failure keeps text for retry", () => {
    ...
    expect(result.current).toMatchObject({
      draft: "On my way",
      state: { sending: false, error: "Not connected" },
    });
    act(() => result.current.send());
    act(() =>
      result.current.handleEvent({
  ...
  it("clears host conversation content on disconnect without losing a browser draft", () => {
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_threads",
        threads: [{ thread, name: "Ada", preview: "private", unread: 1 }],
      }),
    );
    act(() => result.current.selectThread(thread));
    act(() =>
  ...
  it("drops the previous phone's messages when MAP closes", () => {
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_threads",
        threads: [{ thread, preview: "private" }],
      }),
    );
    act(() => result.current.selectThread(thread));
    act(() =>
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_threads",
        threads: [{ thread, preview: "late reply" }],
      }),
    );
    expect(result.current.state.threads).toEqual([]);
  });
  ...
  it("keeps an uncertain draft after timeout and after disconnect", () => {
    ...
    try {
      ...
      expect(result.current).toMatchObject({
        draft: "Do not lose this",
        state: { mapOpen: false, sending: false },
      });
      act(() =>
        result.current.handleEvent({
          command: "bt_send_result",
