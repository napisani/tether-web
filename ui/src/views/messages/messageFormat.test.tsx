...
import { MessageText, sameLocalDay } from "./messageFormat";

describe("message formatting", () => {
  it("links web addresses without treating message text as HTML", () => {
    render(<MessageText body={'Visit https://example.com/path, then <script>alert(1)</script> or www.example.org.'} />);
    expect(screen.getByRole("link", { name: "https://example.com/path" })).toHaveAttribute("href", "https://example.com/path");
    expect(screen.getByRole("link", { name: "www.example.org" })).toHaveAttribute("href", "http://www.example.org");
