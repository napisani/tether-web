...
import { dayHeading, sameLocalDay } from "./messageDates";
  ...
  return (
    <li className={`message-row ${message.outgoing ? "outgoing" : "incoming"}`}>
      <div
        ...
        aria-label={`${message.outgoing ? "Sent" : "Received"}: ${message.body}`}
      >
        <MessageText body={message.body} />
        {showTime && (
          <time
            dateTime={
              message.timestamp > 0 ? new Date(message.timestamp * 1000).toISOString() : undefined
            }
          >
...
function ThreadSidebar({
  ...
  const threads = state.threads.filter((thread) =>
    `${thread.name || ""} ${thread.address || ""} ${thread.preview || ""}`
      .toLocaleLowerCase()
      .includes(search),
  );
  ...
  return (
    <aside className="messages-sidebar" aria-label="Conversations">
      <div className="messages-sidebar-heading">
        <h1>Messages</h1>
        <button type="button" onClick={messages.startCompose} disabled={!daemonConnected}>
          New message
        </button>
      </div>
      ...
      {!daemonConnected && (
        <p className="messages-empty">Reconnect to tetherd to see conversations.</p>
      )}
      {daemonConnected && !state.mapOpen && !state.selected && !state.composing && (
        <div className="messages-sidebar-guidance">
          ...
          {state.permissionOffer && (
            <button type="button" onClick={messages.solicitPermissions}>
              Show iPhone Permissions
            </button>
          )}
        </div>
      )}
      {daemonConnected && state.mapOpen && !state.threadsKnown && (
      ...
      <ul className="messages-threads quiet-scrollbar">
        {threads.map((thread) => (
          <li key={thread.thread}>
            <button
              type="button"
              className={state.selected === thread.thread && !state.composing ? "selected" : ""}
              aria-current={
                state.selected === thread.thread && !state.composing ? "true" : undefined
              }
            ...
            >
              ...
              <span className="messages-thread-meta">
                ...
                {(thread.unread ?? 0) > 0 && (
                  <span className="messages-unread" aria-label={`${thread.unread} unread messages`}>
                    {thread.unread}
                  </span>
                )}
              </span>
...
function RecipientPicker({ messages }: { messages: Messages }) {
  ...
  // Compose is entered by the user; focus the recipient only for that transition.
  useEffect(() => {
    if (state.composing) recipientRef.current?.focus();
  }, [state.composing]);
  ...
  return (
    <div className="messages-recipient">
      ...
      <input
        ...
        onChange={(event) => messages.setRecipient(event.target.value)}
      />
          ...
          {state.contacts.flatMap((contact) =>
            contact.addresses.map((address) => (
              <li key={`${contact.name}:${address}`}>
                <button
                  type="button"
                  onClick={() => messages.chooseRecipient(address, contact.name)}
                >
  ...
  return (
    <div className="messages-composer">
      ...
      <button
        ...
        disabled={!messages.canSend || !messages.draft.trim()}
        onClick={messages.send}
      >
        {messages.state.sending ? "Sending…" : "Send"}
      </button>
    </div>
  );
}
...
function Conversation({
  ...
  useEffect(() => {
    ...
    if (conversation.current && (changed || pinned.current))
    ...
    <section className="messages-conversation" aria-label="Conversation">
      ...
      {!state.mapOpen && (
        <div className="messages-banner" role="status">
          <span>
            {!daemonConnected ? "The Tether daemon is not running." : state.connectionReason}
          </span>
          {daemonConnected && state.permissionOffer && (
            <button type="button" onClick={messages.solicitPermissions}>
              Show iPhone Permissions
            </button>
          )}
      ...
      <ol
        className="messages-history quiet-scrollbar"
        ref={conversation}
        onScroll={(event) => {
      ...
      >
        {state.loadedThread === messages.selectedThread &&
          state.messages.map((message, index) => {
            const previous = state.messages[index - 1];

            const newDay =
              message.timestamp > 0 &&
              (!previous || !sameLocalDay(previous.timestamp, message.timestamp));
            ...
            return (
              <Fragment key={message.handle || `${message.timestamp}:${index}`}>
      ...
      {!state.mapOpen && (
        <p className="messages-composer-notice">Connect Messages on your iPhone before sending.</p>
      )}
      {selected?.repliable === false && (
        <p className="messages-composer-notice">
          {selected.reply_reason || "Replying to this conversation is not available."}
        </p>
      )}
...
export function MessagesView({
  ...
  if (!available) {
    return (
      <main className="messages-view messages-unavailable">
        <h1>Messages</h1>
        <p>This version of tetherd does not advertise Messages support.</p>
      </main>
    );
  }
  ...
  return (
    <main
      className={
        messages.selectedThread || messages.state.composing
          ? "messages-view conversation-open"
          : "messages-view"
      }
    >
