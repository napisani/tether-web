  ...
  act(() => {
    ...
    FakeEventSource.current.emit({
      ...
      threads: [
        ...
        {
          thread: "group:friends",
          name: "Friends",
          preview: "Lunch?",
          repliable: false,
          reply_reason: "Group replies are unavailable.",
        },
      ],
    });
  });
...
describe("Messages view", () => {
  it("navigates, searches, loads a conversation, and confirms only its own send", async () => {
    ...
    await waitFor(() =>
      expect(commandNames()).toContainEqual({
  ...
  it("uses daemon contact suggestions for a new message", async () => {
    ...
    act(() =>
      FakeEventSource.current.emit({
        command: "bt_contacts",
        query: "Ada",
        contacts: [{ name: "Ada", addresses: [thread] }],
      }),
    );
  ...
  it("offers permission recovery only for a withheld Messages profile", () => {
    ...
    act(() => {
      ...
      FakeEventSource.current.emit({
        ...
        map_open: false,
        map_error: "forbidden",
        profile_reason: "Enable Messages in iPhone Bluetooth settings.",
      });
    });
    fireEvent.click(screen.getByRole("button", { name: "Messages" }));
