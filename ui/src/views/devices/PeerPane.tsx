...
function routeDetail(peer: WifiPeer, wifi: WifiState, clipboardAvailable: boolean): string {
  if (peer.connected && peer.paired) {
    if (!clipboardAvailable)
      return (
        "Files and one-time codes are connected. " +
        "This tetherd version does not advertise clipboard sync."
      );

    return wifi.clipboardAvailable
      ? "Clipboard, files, and one-time codes are connected."
      : "Files and one-time codes are connected. The host compositor has no clipboard access; " +
          "desktop clipboard sync is unavailable.";
  }

  if (!wifi.mdnsAvailable)
    return "avahi-daemon is not running, so other devices cannot find this computer.";
    ...
    <div className="device-pane-content peer-pane">
      ...
      <section className="status-section" aria-labelledby="wifi-route-title">
        <div className="section-heading">
          <h3 id="wifi-route-title">Wi-Fi route</h3>
      ...
      <div className="actions">
        {peer.paired ? (
          <button className="secondary-button danger" type="button" onClick={onForget}>
            Forget device
          </button>
        ) : peer.pending ? (
          <button
          ...
          <button
            ...
            disabled={Boolean(wifi.pairingFingerprint)}
          >
            {wifi.pairingFingerprint === peer.fingerprint
        ...
        <p>
          {peer.paired
            ? peer.connected
              ? clipboardAvailable && wifi.clipboardAvailable
            ...
            : peer.pending
              ? "Confirm that you recognize this device before trusting it."
              : "Approval is required on the other device."}
        </p>
      </div>
      {wifi.message ? (
...
export function ConfirmPeerForgetDialog({
  name,
  onCancel,
  onConfirm,
}: {
  ...
  return (
    <ModalDialog labelledBy="forget-peer-title" onCancel={onCancel}>
      ...
      <p>
        Tether will stop trusting this device. Forget this computer on the other device too before
        pairing again.
      </p>
