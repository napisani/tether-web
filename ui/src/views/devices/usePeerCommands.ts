...
export function usePeerCommands(dispatch: Dispatch<AppAction>) {
  ...
  const discover = useCallback(() => {
    ...
    void send({ command: "discover" }, (message) =>
      dispatch({ type: "peer-discovery-failed", token, message }),
    );
  }, [dispatch, send]);
  ...
  const pair = useCallback(
    (peer: WifiPeer) => {
      ...
      void send(
        ...
        (message) =>
          dispatch({ type: "peer-pair-failed", fingerprint: peer.fingerprint, token, message }),
      );
    },
    [dispatch, send],
  );
