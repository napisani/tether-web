...
export function PairingCodeDialog({
  ...
  return (
    <ModalDialog labelledBy="pairing-dialog-title" onCancel={() => onAnswer(false)}>
      ...
      <p>
        Only continue when every digit matches. A different code means this is not the same pairing
        request.
      </p>
      <div className="dialog-actions">
        <button className="secondary-button" type="button" onClick={() => onAnswer(false)}>
          Cancel pairing
        </button>
        <button className="primary-button" type="button" onClick={() => onAnswer(true)}>
          Codes match
        </button>
      </div>
    </ModalDialog>
...
export function ConfirmForgetDialog({
  name,
  onCancel,
  onConfirm,
}: {
  ...
  return (
    <ModalDialog labelledBy="forget-dialog-title" onCancel={onCancel}>
      ...
      <p>
        Also remove this computer from the iPhone’s Bluetooth settings before pairing again.
        Otherwise the phone can keep the old bond.
      </p>
      <div className="dialog-actions">
