...
const status: BluetoothStatusEvent = { command: "bt_status", available: true, enabled: true,
  device_address: "AA:BB", ancs_enabled: true, ancs_content_enabled: true, calls_enabled: false,
  retention: "encrypted", retention_ready: true };

type Settings = ReturnType<typeof useSettings>;

function model(partial: Partial<Settings["state"]> = {}): Settings {
  return { state: { status, pending: null, error: "", ...partial },
    refresh: vi.fn(), handleEvent: vi.fn(), handleDisconnect: vi.fn(), toggle: vi.fn(), setRetention: vi.fn(), resolveUncertain: vi.fn() };
}

afterEach(() => vi.restoreAllMocks());

describe("Settings view", () => {
  it("sends host controls but cannot mistake host popups or session locking for browser features", () => {
    const settings = model();

    render(<SettingsView settings={settings} daemonConnected onOpenDevices={vi.fn()} />);
  ...
  it("confirms irreversible deletion and warns about plaintext before requesting retention", () => {
    ...
    render(<SettingsView settings={settings} daemonConnected onOpenDevices={vi.fn()} />);
    fireEvent.change(screen.getByRole("combobox", { name: "Keep message history" }), { target: { value: "none" } });
    ...
    fireEvent.change(screen.getByRole("combobox", { name: "Keep message history" }), { target: { value: "plaintext" } });
  ...
  it("honors Settings capability negotiation and GTK's legacy enabled default", () => {
    ...
    const { rerender } = render(<SettingsView settings={settings} daemonConnected available={false} onOpenDevices={vi.fn()} />);
    ...
    rerender(<SettingsView settings={settings} daemonConnected available={false} checking onOpenDevices={vi.fn()} />);
    expect(screen.getByText(/Checking tetherd Settings support/)).toBeInTheDocument();
    rerender(<SettingsView settings={settings} daemonConnected available onOpenDevices={vi.fn()} />);
  ...
  it("gates host actions when offline or unbonded, with an explicit recovery path", () => {
    ...
    const { rerender } = render(<SettingsView settings={model({ status: null })} daemonConnected={false} onOpenDevices={onOpenDevices} />);
    ...
    rerender(<SettingsView settings={model({ status: { ...status, device_address: "" } })} daemonConnected onOpenDevices={onOpenDevices} />);
    ...
    rerender(<SettingsView settings={model({ pending: { setting: "retention", expected: "none", phase: "uncertain" }, error: "Check host" })}
      daemonConnected onOpenDevices={onOpenDevices} />);
