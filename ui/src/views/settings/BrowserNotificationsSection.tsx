...
export function BrowserNotificationsSection({ alerts }: { alerts: BrowserNotifications }) {
  ...
  return <section className="settings-group" aria-labelledby="settings-browser-notifications">
    ...
    <div className="settings-row"><label htmlFor="browser-alerts"><strong>Notify when a new iPhone alert arrives</strong>
      <small>This preference stays in this browser, not on the tetherd host.</small></label>
      <input id="browser-alerts" type="checkbox" role="switch" aria-checked={enabled}
        checked={enabled} disabled={unavailable}
        onChange={(event) => { if (event.target.checked) void alerts.enable(); else alerts.disable(); }} />
    </div>
