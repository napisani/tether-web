...
describe("Contacts lifecycle", () => {
  it("loads a bounded list on PBAP open, ignores suggestions and refreshes on return", () => {
    const { result, rerender } = renderHook(({ visible }) => useContacts(visible), {
    ...
    act(() =>
      result.current.handleEvent({ command: "bt_contacts", query: "Ad", contacts: [contact] }),
    );
    expect(result.current.state.loaded).toBe(false);
    act(() =>
      result.current.handleEvent({ command: "bt_contacts", query: "", contacts: [contact] }),
    );
  ...
  it("does not let an earlier failed HTTP refresh cancel a newer refresh timeout", async () => {
    ...
    expect(result.current.state).toMatchObject({
      loading: false,
      error: "Contacts did not respond. Try refreshing.",
    });
  });
