...
import type { CallActionCommand, CallConnectionStatus, DaemonEvent, PhoneCall } from "../../protocol";
...
const initialState: CallsState = { calls: [], loaded: false, status: null, number: "", pending: {}, error: "" };
...
function canSendAction(action: CallAction, path: string, value: CallsState) {
  if (!value.status?.available || value.pending[callActionKey(action, path)] ||
    (path && Object.values(value.pending).some((attempt) => attempt.path === path))) return false;

  if (action === "dial") return Boolean(value.number.trim());

  if (action === "audio_here") return Boolean(value.status.audio) && value.status.audio !== "active";

  if (action === "audio_phone") return value.status.audio === "active";
  const call = value.calls.find((entry) => entry.path === path);

  return Boolean(path && call && call.state !== "disconnected" && (action !== "answer" || call.ringing));
}
...
export function useCalls(visible: boolean) {
  ...
  const refresh = useCallback(() => {
    ...
    void sendDaemonCommand({ command: "bt_list_calls" }).catch(() => {
      if (requestEpoch === epoch.current && visibleRef.current && online.current) {
        change((value) => ({ ...value, error: "Could not refresh calls. Check the tetherd connection." }));
      }
    });
  }, [change]);
  ...
  const resetConnection = useCallback((reason: string) => {
    hfpReady.current = false;
    epoch.current++;

    for (const key of timers.current.keys()) clearTimer(key);
    change((value) => ({ ...initialState, number: value.number,
      status: { available: false, reason },
      error: Object.values(value.pending).length
        ? "Connection lost; a call action may have reached your iPhone. Check it before trying again."
  ...
  const applyCallList = useCallback((calls: PhoneCall[]) => {
    ...
    change((value) => ({ ...value, calls, loaded: true, pending,
      error: Object.keys(pending).length === 0 && Object.keys(value.pending).length > 0 ? "" : value.error }));
  }, [change, clearTimer]);

  const applyConnectionStatus = useCallback((status: CallConnectionStatus | null) => {
    ...
    for (const [key, attempt] of Object.entries(pending)) {
      const routed = (attempt.action === "audio_here" && status.audio === "active") ||
    ...
    change((value) => ({ ...value, status, pending,
      error: Object.keys(pending).length === 0 && Object.keys(value.pending).length > 0 ? "" : value.error }));
  ...
  const resolveUncertain = () => change((value) => ({ ...value, error: "",
    pending: Object.fromEntries(Object.entries(value.pending).filter(([, attempt]) => attempt.status !== "uncertain")) }));
  ...
  const sendAction = (action: CallAction, path = "") => {
    ...
    change((previous) => ({ ...previous, error: "", pending: { ...previous.pending,
      [key]: { action, path, status: "waiting" } } }));
    timers.current.set(key, window.setTimeout(() => {
      if (!current.current.pending[key] || current.current.pending[key].status === "uncertain") return;
      timers.current.delete(key);
      change((previous) => ({ ...previous, pending: { ...previous.pending,
        [key]: { ...previous.pending[key], status: "uncertain" } },
      error: "No confirmation from the iPhone. Check the call before trying again." }));
    }, actionTimeoutMs));

    const command: CallActionCommand | { command: "bt_call_dial"; number: string } = action === "dial"
    ...
    void sendDaemonCommand(command).catch(() => {
      if (current.current.pending[key]) change((previous) => ({ ...previous,
