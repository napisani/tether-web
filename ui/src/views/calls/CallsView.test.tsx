...
function connect(available = true) {
  act(() => {
    ...
    FakeEventSource.current.emit({ command: "protocol_info", version: 1, capabilities: available ? ["calls"] : [] });
  });
}
...
describe("Calls view", () => {
  it("explains the host-side opt-in and HFP availability", () => {
    ...
    act(() => FakeEventSource.current.emit({ command: "bt_connection_changed", calls: {
      available: false, reason: "The iPhone has not connected Hands-Free." } }));
  ...
  it("displays withheld callers, answers and declines by path, and labels host audio accurately", () => {
    ...
    act(() => {
      FakeEventSource.current.emit({ command: "bt_connection_changed", calls: {
        available: true, audio: "idle", indicators: true, operator: "Carrier", service: true, signal: 4, battery: 3 } });
      FakeEventSource.current.emit({ command: "bt_calls", calls: [
    ...
    expect(commands()).toContainEqual({ command: "bt_call_action", action: "answer", path: "/call/1" });
    act(() => FakeEventSource.current.emit({ command: "bt_call_result", action: "answer", success: false, message: "Could not answer." }));
    expect(screen.getByRole("button", { name: "Decline call with Withheld number" })).toBeDisabled();
    act(() => FakeEventSource.current.emit({ command: "bt_calls", calls: [
    ...
    expect(commands()).toContainEqual({ command: "bt_call_action", action: "hangup", path: "/call/1" });
    ...
    act(() => FakeEventSource.current.emit({ command: "bt_connection_changed", calls: { available: true, audio: "active" } }));
  ...
  it("preserves the dial number on global results and clears private call rows on disconnect", () => {
    ...
    act(() => FakeEventSource.current.emit({ command: "bt_call_result", action: "dial", success: true }));
    expect(number).toHaveValue("+15550100");
    act(() => FakeEventSource.current.emit({ command: "bt_calls", calls: [{ path: "/call/1", state: "dialing", outgoing: true, number: "+15550100" }] }));
