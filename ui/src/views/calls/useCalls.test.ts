...
const ready: CallConnectionStatus = {
  available: true,
  audio: "idle",
  indicators: true,
  operator: "Carrier",
...
describe("Calls lifecycle", () => {
  it("refreshes on visibility and HFP availability, but ignores lists while unavailable", () => {
    const { result, rerender } = renderHook(({ visible }) => useCalls(visible), {
    ...
    act(() =>
      result.current.handleEvent({
        ...
        calls: { available: false, reason: "Hands-Free not connected." },
      }),
    );
    act(() => result.current.handleEvent({ command: "bt_calls", calls: [incoming] }));
  ...
  it("does not attribute global results to a dial; requires explicit retry", () => {
    vi.useFakeTimers();
    ...
    act(() =>
      result.current.handleEvent({ command: "bt_call_result", action: "hangup", success: true }),
    );
    expect(result.current.state.pending.dial?.status).toBe("waiting");
    act(() =>
      result.current.handleEvent({ command: "bt_call_result", action: "dial", success: true }),
    );
    expect(result.current.state.pending.dial?.status).toBe("waiting");
    act(() =>
      result.current.handleEvent({ command: "bt_call_result", action: "dial", success: false }),
    );
    expect(result.current.state.pending.dial?.status).toBe("waiting");
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_calls",
        calls: [incoming, { path: "/other-call", state: "dialing", outgoing: true }],
      }),
    );
    expect(result.current.state.pending.dial?.status).toBe("waiting");
    act(() => {
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_calls",
        calls: [incoming, { path: "/call/2", state: "dialing", outgoing: true }],
      }),
    );
    expect(result.current.state.pending.dial?.status).toBe("waiting");
    expect(result.current.state.number).toBe("+1 (555) 0100");
  ...
  it("scopes actions by call path and resets pending work on disconnect", () => {
    const { result } = renderHook(() => useCalls(false));
    connect(result);
    act(() =>
      result.current.handleEvent({
        command: "bt_calls",
        calls: [incoming, { path: "/call/2", state: "active", connected: true }],
      }),
    );
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_calls",
        calls: [{ ...incoming, state: "active", ringing: false }],
      }),
    );
  ...
  it("does not let a global result resolve two independent call rows", () => {
    ...
    act(() =>
      result.current.handleEvent({ command: "bt_call_result", action: "answer", success: false }),
    );
    expect(result.current.state.pending[`answer:${incoming.path}`]?.status).toBe("waiting");
    ...
    act(() =>
      result.current.handleEvent({
        command: "bt_calls",
        calls: [{ ...incoming, ringing: false }, second],
      }),
    );
    expect(result.current.state.pending[`answer:${incoming.path}`]).toBeUndefined();
    expect(result.current.state.pending[`answer:${second.path}`]?.status).toBe("waiting");
