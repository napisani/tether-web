...
const open = <Fields extends Record<string, z.ZodTypeAny>>(fields: Fields) =>
...
export const bluetoothConnectionEventSchema = open({
  ...
  calls: open({
    ...
    indicators: optionalBoolean,
    operator: optionalString,
    service: optionalBoolean,
    signal: z.number().optional(),
    roaming: optionalBoolean,
...
const threadListEventSchema = open({
  command: z.literal("bt_threads"),
  threads: z.array(messageThreadSchema),
});

const messageListEventSchema = open({
  command: z.literal("bt_messages"),
  thread: z.string().min(1),
  messages: z.array(open(textMessageSchema)),
});

const newMessageEventSchema = open({ command: z.literal("bt_message"), ...textMessageSchema });

const messageSendResultEventSchema = open({
  command: z.literal("bt_send_result"),
  operation_id: operationId,
  thread: z.string().min(1),
  success: z.boolean(),
  message: optionalString,
...
const messageReadEventSchema = open({
  command: z.literal("bt_message_read"),
...
const notificationListEventSchema = open({
  command: z.literal("bt_notifications"),
  notifications: z.array(open(notificationFields)),
});

const newNotificationEventSchema = open({
  command: z.literal("bt_notification"),
  ...notificationFields,
});

const notificationRemovedEventSchema = open({
  command: z.literal("bt_notification_removed"),
  uid: notificationUid,
});

const notificationActionResultEventSchema = open({
  command: z.literal("bt_notification_action_result"),
  uid: notificationUid,
  success: z.boolean(),
});
...
const phoneCallSchema = open({
  ...
  withheld: optionalBoolean,
  ringing: optionalBoolean,
  connected: optionalBoolean,
  outgoing: optionalBoolean,
  incoming_line: optionalString,
  multiparty: optionalBoolean,
});

const callListEventSchema = open({
  command: z.literal("bt_calls"),
  calls: z.array(phoneCallSchema),
});

const callResultEventSchema = open({
  command: z.literal("bt_call_result"),
  action: z.string(),
  success: z.boolean(),
  message: optionalString,
});
...
export const daemonCommandSchema = z.discriminatedUnion("command", [
  ...
  open({
    command: z.literal("bt_pair"),
    address: z.string().min(1),
    operation_id: z.string().min(1),
  }),
  open({
    command: z.literal("bt_unpair"),
    address: z.string().min(1),
    operation_id: z.string().min(1),
  }),
  open({
    command: z.literal("bt_pair_confirm"),
    operation_id: z.string().min(1),
    accept: z.boolean(),
  }),
  ...
  open({
    command: z.literal("bt_set_retention"),
    retention: z.enum(["encrypted", "plaintext", "none"]),
  }),
  ...
  open({
    command: z.literal("bt_airpods_mode"),
    mode: z.enum(["off", "transparency", "adaptive", "anc"]),
  }),
  open({
    command: z.literal("bt_airpods_connect"),
    address: z.string().min(1),
    connect: z.boolean(),
  }),
  open({ command: z.literal("discover") }),
  open({
    command: z.literal("pair_request"),
    host: z.string().min(1),
    port: tcpPort,
    device_name: z.string().min(1),
  }),
  open({
    command: z.literal("accept_device"),
    fingerprint: z.string().min(1),
    device_name: z.string().min(1),
  }),
  open({ command: z.literal("forget_device"), fingerprint: z.string().min(1) }),
  open({
    command: z.literal("file_upload_start"),
    operation_id: z.string().min(1),
    filename: z.string().min(1),
    size: z.number().finite().nonnegative(),
  }),
  open({
    command: z.literal("file_upload_chunk"),
    operation_id: z.string().min(1),
  ...
  open({
    command: z.literal("bt_list_contacts"),
    query: z.string(),
    limit: z.number().int().positive().optional(),
  }),
  open({
    command: z.literal("bt_mark_read"),
    handles: z.array(z.string().min(1)).min(1),
    read: z.literal(true),
  }),
  open({
    command: z.literal("bt_send_message"),
    thread: z.string().min(1),
    body: z.string().min(1),
    operation_id: z.string().min(1),
  }),
  ...
  open({
    command: z.literal("bt_notification_action"),
    uid: notificationUid,
    action: z.literal("negative"),
  }),
  ...
  open({
    command: z.literal("bt_call_action"),
    action: z.enum(["answer", "hangup", "audio_here", "audio_phone"]),
    path: optionalString,
  }),
]);
