...
describe("parseDaemonEvent", () => {
  it("validates protocol capabilities before exposing them to the UI", () => {
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "protocol_info", version: 1, capabilities: null }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(JSON.stringify({ command: "protocol_info", version: 0, capabilities: [] })),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({
  ...
  it("rejects malformed peer lifecycle and discovery events", () => {
    expect(
      parseDaemonEvent(JSON.stringify({ command: "pair_accepted", connected: true })),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "discovery_result", devices: [{ name: "Phone" }] }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
          ...
          devices: [
            {
              name: "Phone",
              fingerprint: "peer",
              addresses: [{ address: "10.0.0.2", port: 70_000 }],
            },
          ],
        }),
      ),
  ...
  it("validates Bluetooth status, diagnostics, and solicitation results", () => {
    expect(
      parseDaemonEvent(JSON.stringify({ command: "bt_status", available: "yes" })),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(JSON.stringify({ command: "bt_status", available: true, enabled: "yes" })),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(JSON.stringify({ command: "bt_status", available: true, ancs_enabled: 1 })),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({
          ...
          capability: {
            mode: "compatibility",
            reasons: [],
            setup: [{ what: "Enable support", command: 42 }],
          },
        }),
      ),
    ).toBeUndefined();
    ...
    expect(
      parseDaemonEvent(JSON.stringify({ command: "bt_solicit_result", success: true })),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({
  ...
  it("rejects malformed file-transfer result events", () => {
    expect(
      parseDaemonEvent(JSON.stringify({ command: "file_send_complete", success: true })),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({
  ...
  it("validates notification UIDs, lists, and action results", () => {
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_notifications", notifications: [{ uid: -1 }] }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_notifications", notifications: [{ uid: 42, title: 123 }] }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(JSON.stringify({ command: "bt_notification_removed", uid: "42" })),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_notification_action_result", uid: 42, success: "yes" }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({
          ...
          notifications: [{ uid: 42, title: "Hello" }],
        }),
      ),
    ).toMatchObject({ command: "bt_notifications", notifications: [{ uid: 42, title: "Hello" }] });
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_notification_action_result", uid: 42, success: true }),
      ),
  ...
  it("validates daemon-owned settings without discarding legacy status events", () => {
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_status", available: true, retention: "erase" }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_status", available: true, ancs_content_enabled: "yes" }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({
          ...
          lock_on_away: true,
          desktop_popups_enabled: false,
        }),
      ),
    ).toMatchObject({ retention: "encrypted", retention_ready: false, lock_on_away: true });
    expect(
      parseDaemonEvent(JSON.stringify({ command: "bt_status", available: true })),
  ...
  it("validates Hands-Free status, call lists, and global action results", () => {
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_connection_changed", calls: { available: "yes" } }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(JSON.stringify({ command: "bt_calls", calls: [{ state: "incoming" }] })),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_call_result", action: "dial", success: "yes" }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
        JSON.stringify({
          command: "bt_calls",
          calls: [{ path: "/call/1", withheld: true, number: "" }],
        }),
      ),
  ...
  it("validates Messages payloads before rendering them", () => {
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_threads", threads: [{ name: "Missing key" }] }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
    ...
    expect(
      parseDaemonEvent(
        JSON.stringify({ command: "bt_send_result", thread: "tel:+15550102", success: "yes" }),
      ),
    ).toBeUndefined();
    expect(
      parseDaemonEvent(
...
describe("sendDaemonCommand", () => {
  it("rejects unsupported call actions before contacting tetherd", async () => {
    ...
    await expect(
      sendDaemonCommand({ command: "bt_call_action", action: "hold_and_answer" } as never),
  ...
  it("rejects unsupported retention modes and accepts documented host commands", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    await expect(
      sendDaemonCommand({ command: "bt_set_retention", retention: "delete" } as never),
    ).rejects.toThrow("invalid tetherd command");
    expect(fetchMock).not.toHaveBeenCalled();
